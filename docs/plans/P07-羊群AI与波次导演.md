...
- [ ] 写 `config/sheep.ts`：四羊形照抄需求 §7.3；追加 AI 参数（`sightM` 35、`attackRangeM` 1.4、`attackCooldownMs` 1200、`chargeWindupMs` 1000、`chargeSpeedMps` 9、`eliteBoltRangeM` 25、`eliteBoltCooldownMs` 2500、`kingSummonCount` 4）。
- [ ] 写 `steering.ts`：每个函数输入 `(self, target, neighbors, world)`，输出期望速度向量（不修改实体），便于单测与复用。
- [ ] 写 `flocking.ts`：权重 `separation 1.6 / alignment 0.4 / cohesion 0.5`；邻居半径 3m、最大邻居数 12（按距离取最近，避免 O(n²) 的排序开销累积）；力向量写入调用方传入的临时对象。
- [ ] 写 `targeting.ts`：目标 = 最近的"可见"玩家（可见性用 P05 的 `rayVsAabb` 做遮挡检测）；仇恨表 `Map<pid, aggro>` 每 tick 衰减 2%，受击 +20，用于让羊不无限换目标（避免抖动）。
- [ ] 写 `sheepBrain.ts`：状态机与转移矩阵（§5.1）；`grunt` 用 `graze → alert → chase → attack`；`ram` 增加 `windup → charge`；`elite` 增加 `ranged`（保持 15–25m 距离并发射问号弹）；`king` 由 `kingPhases.ts` 覆盖。
- [ ] 写 `sheepAttack.ts`：近身撕咬（1.2s 冷却、8 伤害、命中后玩家被击退 1.5m）；冲锋命中（22 伤害 + 击退 3m + 冲锋者自身硬直 1s）；问号弹（速度 14 m/s、伤害 14、命中或撞墙销毁、可被玩家射击提前引爆——P1 效果好则保留）。所有伤害走 P06 的 `damage.ts` 与事件生产责任表。
- [ ] 写 `kingPhases.ts`：阶段 1（生命 100%–66%）冲撞；阶段 2（66%–33%）每 8 秒召唤 4 只咩咩兵；阶段 3（<33%）移速 ×1.4、攻击冷却 ×0.7。
- [ ] 写 `director.ts`：`budget(wave) = round(6 + 3.2 * wave + 0.18 * wave^2)`，人数系数 `1 + 0.35 * (playerCount - 1)`；羊形价格（需求 §7.3）；组队规则——前 2 波只出 `grunt`，第 3 波起引入 `ram`，第 5 波起引入 `elite`，`wave % 5 == 0` 必出 1 只 `king`；生成时在 12 个生成点中挑选"离最近玩家 > 15m"的点，每波最多 3 个生成点同时使用；每 tick 最多生成 8 只（防止单帧雪崩）；波次在预算耗尽且场上无羊时判清空。
- [ ] 在 `sim.ts` 第 2 槽调用 AI 求解（顺序：先计算所有意图，再统一写入，保证与实体遍历顺序无关）、第 5 槽推进投射物。
- [ ] 客户端：冲锋预警（地面红环 + 音效预兆由 P09 补音）、问号弹的可辨识表现（旋转的问号造型，用简单几何 + 自发光）、羊王血条。
- [ ] 测试：
  - [ ] 转向：`separation` 在两点重合时输出非零向量；`arrive` 在目标距离内速度收敛到 0。
  - [ ] 群体：20 只羊聚集在 3m 内时，分离力使最近两点距离在下个 tick 增大。
  - [ ] 状态迁移矩阵：每个状态列出允许的下一状态，非法迁移在单测中不存在。
  - [ ] 预算：波次 1/5/10 在 1 人与 4 人下的预算值与需求 §7.4 一致；组队结果的价格总和 ≤ 预算。
  - [ ] 确定性：同种子跑 1000 tick，两次羊群位置逐位一致；`grep -rn "Math.random" packages/shared/src/ai` 无结果。
  - [ ] Boss：羊王按血量阈值进入阶段 2 并生成恰好 4 只咩咩兵。
