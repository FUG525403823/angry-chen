...
//
// 冻结段序：通用包头 → 实体块（count u8 + 15 B × n，id 升序，≤255）→ 移除列表（removedCount u8 +
...
inline constexpr std::size_t kSnapshotCapacityBytes = 2048u;     // §5 单快照上限（含包头）
inline constexpr std::size_t kSteadySnapshotBudgetBytes = 1228u; // §5 稳态快照上限
...
// 世界 → 线上记录（按 EntityId 升序；容量不足时截断并返回实际条数）。
