...
> 当前仓库处于**规划完成、尚未开始编码**的状态：这里只有文档与一个文档链校验脚本，按文档施工从 P01 开始。
...
```bash
node tools/check-docs.mjs     # ① 校验十份计划文档的线性链是否完整
# ② 从 P01 开始施工；P01 完成后本仓库将提供：
#    pnpm install
#    pnpm check      # typecheck + lint + test + 文档链校验
#    pnpm dev        # 并行启动服务端与客户端
```
